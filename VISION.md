...
This document defines durable direction. Implementation details should evolve, but all work should align with these principles. Moorline should not be framed around one application category, provider family, transport style, deployment location, or workflow domain.
...
- Make Moorline extensible across many event sources, resources, interaction surfaces, providers, and workflow ideas without making any one surface the architectural center.
- Favor strong defaults over broad configuration in early and growth stages.
- Let operators manage the runtime through replaceable operator surfaces with equivalent core management coverage.
...
- `api adapters` expose Moorline control and management contracts through protocol-specific packages.
- `transports` connect Moorline to external event sources, resources, and interaction environments.
...
- Operator surfaces are replaceable clients of the same runtime management contracts.

## Core Invariants

- Operator control of runtime state, packages, configuration, policy, and audit, independent of deployment location.
...
- Moorline should support many event sources, external resources, interaction surfaces, and operator surfaces without making any single surface the architectural center of the system.
- Runtime flow should center on external events/resources, durable work, provider execution, gates/actions, audit, and results or escalations.
...
- Treat memory as a first-class subsystem with provenance, scope, and lifecycle.
- Keep layered memory:
  - durable facts
  - work, session, or resource-scoped memory
...
- Letting a transport, provider, plugin, API adapter, operator surface, or workflow package become a substitute for the core runtime boundary.
- Defining Moorline by one deployment location, one interface style, one provider, one integration, or one workflow domain.
