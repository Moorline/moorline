...
- `operator-controlled runtime`
  - Preferred high-level product framing. Moorline is controlled by the person or team running it, regardless of whether it runs on an operator machine, server, or another controlled environment.
- `external surface`
  - A place where work, events, or interaction can enter or leave Moorline, such as external services, event systems, request queues, or an API adapter.
- `work`
  - Durable agent-powered activity owned by the runtime. Work may come from messages, external events, package triggers, scheduled jobs, or operator requests.
- `CLI`
...
- deployment-locality framing
  - Avoid defining Moorline by where it runs. Prefer `operator-controlled`, `self-hostable`, or concrete deployment wording.
- domain-specific operator labels
  - Avoid defining Moorline by one workflow family. Its core shape is external event/resource intake, durable work, provider execution, policy, audit, and package orchestration.
- `coordination-centered`
  - Avoid. Message-oriented transports are one shape, not the center of the runtime architecture.
- `package surfaces`
