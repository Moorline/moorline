...
export class PendingRequestRepository {
  ...
  listPendingRequestsByTransportResource(transportResourceId: string | null | undefined): PendingRuntimeRequestRecord[] {
    ...
    return mapRows<PendingRuntimeRequestRecord>(
      this.db.prepare(`${pendingRequestSelect} WHERE transport_resource_id = ? ORDER BY created_at ASC, request_id ASC`).all(transportResourceId)
    );
  }
  ...
  listOpenPendingRequestsByTransportResource(transportResourceId: string | null | undefined): PendingRuntimeRequestRecord[] {
    ...
    return mapRows<PendingRuntimeRequestRecord>(
      this.db.prepare(`${pendingRequestSelect} WHERE status = 'open' AND transport_resource_id = ? ORDER BY created_at ASC, request_id ASC`).all(transportResourceId)
    );
  }
